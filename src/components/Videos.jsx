...
const VideoCard = ({ src, title, delay }) => (
  <motion.div
...
const Videos = () => {
  ...
  return (
    <section className="relative min-h-screen py-24 px-6 text-center overflow-hidden" id="videos">
      ...
      {/* Section Title */}
      <motion.h2
        className="text-4xl sm:text-5xl font-extrabold mb-4 text-[#0F172A] relative z-10 drop-shadow-[2px_2px_0px_#FACC15]"
            ...
            >
              ...
              <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
                {visibleVideos.map((v, idx) => (
                  <VideoCard key={idx} src={v.src} title={v.title} delay={idx * 0.2} />
                ))}
              </div>
              ...
              {cat.videos.length > 3 && (
                <button
                  ...
                  className="mt-6 text-sm text-[#0F172A] bg-yellow-300 hover:bg-yellow-400 font-semibold px-4 py-2 rounded-full transition-all"
                >
